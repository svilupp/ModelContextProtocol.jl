...
function convert_element(element::AbstractString)::String
    ...
    elseif occursin(r"<ul[^>]*>", element)
        ...
        items = [process_inline(item.captures[1])
                 for item in eachmatch(r"<li[^>]*>(.*?)</li>"s, content)]
...
"""
function html_to_markdown(html::AbstractString)::AbstractString
